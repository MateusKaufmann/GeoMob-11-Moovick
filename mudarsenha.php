...
$status = "<script src='https://unpkg.com/@lottiefiles/lottie-player@latest/dist/lottie-player.js'>
                                ...
                                <lottie-player src='https://assets4.lottiefiles.com/datafiles/Y00QYvz2RSOfBnQ/data.json'
...
// Dados
        ...
        $cpf = mysqli_query($connect, "SELECT cpf FROM usuarios WHERE id = '$id'");
        $resultado = mysqli_query($connect, $sql);
        $dados = mysqli_fetch_array($resultado);
        $senha = mysqli_escape_string($connect, $_POST['senha']);
        $mudarsenha = mysqli_escape_string($connect, $_POST['mudarsenha']);
        $mudarsenha2 = mysqli_escape_string($connect, $_POST['mudarsenha2']);
...
if(isset($_POST['btn-entrar'])):
        ...
        $erros2 = "";
                ...
                if (!empty($mudarsenha)):
                    ...
                    if(strlen($mudarsenha) >= 8) {
        
                        if($mudarsenha == $mudarsenha2):
                
                
                        $sql = "UPDATE usuarios
                        SET senha = MD5('$mudarsenha')
                        WHERE id = '$id'";
			            mysqli_query($connect, $sql);
                        $erros = "<div class='alert alert-success' role='alert'>
                        <i class='fas fa-check-double'></i> Senha modificada.
                        </div>";
                        ...
                        src="https://assets3.lottiefiles.com/datafiles/OhIfcxnkLsj1Jxj/data.json"  background="transparent"  speed="1"  style="width: 4rem; height: 4rem;" autoplay >
                          ...
                          $erros =  "<div class='alert alert-danger' role='alert'>
                            <i class='fas fa-exclamation-triangle'></i> Falha! Senhas não se correspondem
                            </div>";
                            $status = '<script src="https://unpkg.com/@lottiefiles/lottie-player@latest/dist/lottie-player.js"></script>
              <lottie-player 
                  src="https://assets5.lottiefiles.com/temp/lf20_tNI4Yn.json"  background="transparent"  speed="1"  style="width: 4rem; height: 4rem;"  autoplay >
                        ...
                        $erros =  "<div class='alert alert-danger' role='alert'>
                        <i class='fas fa-exclamation-triangle'></i> Falha! Sua senha possui menos de 8 caracteres!
                        </div>";
                        $status = '<script src="https://unpkg.com/@lottiefiles/lottie-player@latest/dist/lottie-player.js"></script>
              <lottie-player 
                  src="https://assets5.lottiefiles.com/temp/lf20_tNI4Yn.json"  background="transparent"  speed="1"  style="width: 4rem; height: 4rem;"  autoplay >
                        ...
                        <div class="panel text-left shadow-lg">
                            ...
                            <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                                <div class="input-group mb-3">
                                    <input type="password" class="form-control" placeholder="Nova Senha" name="mudarsenha"
                                        aria-label="Example text with button addon" aria-describedby="button-addon1">
                                    <input type="password" class="form-control" placeholder="Confirme a Senha"
                                        name="mudarsenha2" aria-label="Recipient's username"
                                        aria-describedby="button-addon2">
                                </div>
                                <div class="input-group mb-3">
                                    <button class="btn btn-outline-secondary" type="submit" name="btn-entrar">Mudar
                                        Senha</button>
                            </form>
